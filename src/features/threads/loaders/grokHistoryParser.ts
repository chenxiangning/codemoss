...
function extractGrokAssistantFinalFlag(message: Record<string, unknown>): boolean | undefined {
  const metadata =
...
function markGrokAssistantFinalMessages(items: ConversationItem[]) {
...
function hydrateGrokAssistantFinalTiming(
...
function normalizeGrokToolSnapshotType(rawToolType: string): string {
...
function normalizeGrokToolStartItem(
  message: Record<string, unknown>,
  itemId: string,
  rawToolType: string,
  text: string,
): Extract<ConversationItem, { kind: "tool" }> | null {
  const normalizedToolType = normalizeGrokToolSnapshotType(rawToolType);
...
export function parseGrokHistoryMessages(messagesData: unknown): ConversationItem[] {
  ...
  for (const entry of messagesData) {
    ...
    const itemId =
      asString(message.id ?? "").trim() || `grok-history-item-${items.length + 1}`;
    ...
    if (kind === "message") {
      ...
      const assistantFinalFlag =
        role === "assistant"
          ? extractGrokAssistantFinalFlag(message)
          : undefined;
    ...
    const normalizedStartItem = normalizeGrokToolStartItem(
      message,
      itemId,
      rawToolType,
      text,
  ...
  markGrokAssistantFinalMessages(items);
  hydrateGrokAssistantFinalTiming(items, messageTimestampById);
