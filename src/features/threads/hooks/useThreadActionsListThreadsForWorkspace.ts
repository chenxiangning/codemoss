...
import type { AutomaticRuntimeRecoverySource } from "./useAutomaticRuntimeRecovery";
import {
...
type CachedSessions<T> = Record<string, { fetchedAt: number; sessions: T[] }>;

export type UseListThreadsForWorkspaceOptions = {
  ...
  getLastGoodThreadSummaries: (workspaceId: string) => ThreadSummary[];
  ...
  geminiSessionCacheRef: MutableRefObject<CachedSessions<GeminiSessionSummary>>;
  ...
  loadActiveProjectCatalogSessions: (
    workspaceId: string,
    mode?: WorkspaceSessionAttributionMode,
  ...
  threadListRequestSeqRef: MutableRefObject<Record<string, number>>;
...
}: UseListThreadsForWorkspaceOptions) {
  return useCallback(
    async (
      ...
      options?: {
        ...
        /**
         * Opt-in engine disk `list_*` fan-out (Claude/Gemini/Grok/Kimi/OpenCode
         * / project catalog). Default false: sidebar hydrate uses Session Index.
         */
      ...
      try {
        ...
        if (typeof listSessionIndexForWorkspaceService === "function") {
          ...
          if (sessionIndexPage === null && isFirstPaintHydration && !forceIndexSync) {
            sessionIndexPage = await withTimeout(
              listSessionIndexForWorkspaceService(workspace.id, {
                limit: sessionIndexLimit,
                syncIfNeeded: false,
          ...
          if (!canMergeIndexNatives) {
            ...
            sessionIndexRowsToThreadSummaries(
              ...
              if (!prev || summary.updatedAt >= prev.updatedAt) {
                mergedById.set(summary.id, prev ? { ...summary, name: prev.name || summary.name } : summary);
              }
            });
          } else {
