...
import { resolveSharedSessionCreateInitialTarget } from "@mossx/plugin-shared-session/runtime";
...
export function useAppShellSections(input: UseAppShellSectionsInput) {
  ...
  const ctx = mergeAppShellDomainBag<UseAppShellSectionsContext>(
    domainBag,
    input.searchAndComposerSection as Record<string, unknown>,
  );
  const {
    activeWorkspace,
    workspaces,
