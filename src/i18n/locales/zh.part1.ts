const zhPart1 = {
  ...
  settings: {
    ...
    runtimePoolTitle: "运行时池控制台",
    runtimePoolDescription: "统一查看受管运行时的热度、预算、诊断与人工干预状态。",
    runtimePanelTitle: "运行时池控制台",
    runtimePanelDescription: "独立管理运行时池，不再埋在其他设置里。这里负责看池子健康度、调容量、手动干预异常实例。",
    runtimeScopeTitle: "当前纳管范围",
    runtimeScopeDescription:
      "当前 Hot / Warm / Cold 池化主要覆盖 Codex。Claude Code 首期先纳入统一退出清理、残留诊断与进程收口，但还没有进入热池/温池复用调度。",
    runtimeBadgeHot: "热实例上限 {{value}}",
    runtimeBadgeWarm: "温实例上限 {{value}}",
    runtimeBadgeTtl: "保温 {{value}} 秒",
    runtimeMetricTotal: "总实例",
    runtimeMetricHot: "热实例",
    runtimeMetricWarm: "温实例",
    runtimeMetricBusy: "忙碌实例",
    runtimeMetricPinned: "固定实例",
    runtimeStateHot: "热实例",
    runtimeStateWarm: "温实例",
    runtimeStateBusy: "忙碌中",
    runtimeStateStarting: "启动中",
    runtimeStateStopping: "停止中",
    runtimeStateFailed: "异常",
    runtimeStateZombie: "疑似残留",
    runtimePolicyTitle: "生命周期策略",
    runtimePolicyDescription: "决定应用启动、退出和下次启动时如何处理后台运行时。",
    runtimeRestoreThreadsOnlyOnLaunch: "启动时只恢复线程元数据",
    runtimeRestoreThreadsOnlyOnLaunchDesc: "恢复工作区里的线程和 UI 状态，但不为所有工作区批量拉起后台运行时，避免冷启动时进程爆炸。",
    runtimeForceCleanupOnExit: "退出时强制清理受管运行时",
    runtimeForceCleanupOnExitDesc: "关闭客户端时主动 drain 受管运行时，优先解决老机器上残留后台进程的问题。",
    runtimeOrphanSweepOnLaunch: "下次启动时清扫残留运行时",
    runtimeOrphanSweepOnLaunchDesc: "启动时扫描上次异常退出遗留的 ledger / orphan runtime，尽量把脏状态清理干净。",
    runtimeBudgetTitle: "容量与保温预算",
    runtimeBudgetDescription: "把运行时数量从“工作区数量驱动”改成“预算驱动”，这是这套方案的核心。",
    runtimeMaxHot: "Codex 热实例上限",
    runtimeMaxHotHelp: "允许同时保持可直接复用的热实例数量。更大更快，但会更吃内存。",
    runtimeMaxWarm: "Codex 温实例上限",
    runtimeMaxWarmHelp: "允许保留的温实例数量。温实例不活跃，但比冷启动更容易回温。",
    runtimeWarmTtl: "温实例 TTL（秒）",
    runtimeWarmTtlHelp: "温实例在无人使用时最多保留多久，超时后自动回收到冷态。",
    runtimePoolSummary: "运行时池摘要",
    runtimeSummaryLine:
      "总数 {{total}} · 热实例 {{hot}} · 温实例 {{warm}} · 忙碌 {{busy}} · 固定 {{pinned}}",
    runtimeDiagnosticsLine:
      "残留已清理 {{cleaned}} · 残留失败 {{failed}} · 强制终止 {{forced}}",
    runtimeRowsTitle: "活跃运行时实例",
    runtimeRowsDescription: "每一行就是一个受管运行时实例。你可以看到它属于哪个工作区、当前热度、进程信息和最近使用情况。",
    ...
    runtimePoolEmpty: "当前没有活跃的受管运行时。",
    runtimeEmptyDescription: "这通常说明当前没有活跃的 Codex 会话，或者实例已经按预算策略回收到冷态。",
    runtimePin: "固定",
    runtimeUnpin: "取消固定",
    runtimeRelease: "释放",
    runtimeClose: "关闭",
