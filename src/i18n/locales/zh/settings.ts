  ...
  settings: {
    ...
    vendor: {
      ...
      piAuth: {
        ...
        oauthDesc: {
          claude: "Anthropic 订阅 · 第三方调用按 token 计量，计入 extra usage",
          codex: "OpenAI 订阅 · 需要 ChatGPT Plus 或 Pro 计划",
          copilot: "支持 github.com 与 GitHub Enterprise Server 域名",
          xai: "订阅授权 · 也可在下方使用 XAI_API_KEY",
          openrouter: "PKCE 授权后签发用户可控 API Key，从 OpenRouter 额度计费",
          radius: "pi-messages 动态网关 · 模型目录独立刷新并缓存"
        },
        subscribed: "已授权 · 自动刷新",
        notSubscribed: "未授权",
        login: "登录",
        apiKeyTitle: "API Key",
        apiKeyHint: "写入 ~/.pi/agent/auth.json · 优先级高于环境变量",
        searchPlaceholder: "筛选供应商…",
        configured: "已配置",
        notConfigured: "未配置",
        envActive: "环境变量生效中",
        envActiveHint: "环境变量生效中（mossx 启动 pi 时继承的环境）。如需覆盖，请设置 auth.json 凭证。",
        setKey: "设置 Key",
        overrideSet: "覆盖设置",
        edit: "编辑",
        delete: "删除",
        ...
        keyPlaceholderNew: "粘贴 {{env}}，例如 sk-…",
        keyPlaceholderKeep: "{{mask}}（留空保持不变）",
        toggleKeyVisibility: "显示 / 隐藏",
        advancedTipPrefix: "高级用法：key 支持 ",
        advancedTipMiddle: " 执行密钥工具（如 !op read 'op://vault/item'）与 ",
        advancedTipSuffix: " 环境变量插值；留空则保持现有凭证。",
        saveHint: "保存后写入 ~/.pi/agent/auth.json（0600 权限）",
        showAll: "显示全部 {{count}} 个供应商",
        showLess: "收起非常用供应商",
        emptySearch: "没有匹配「{{query}}」的供应商",
        loadFailed: "读取认证状态失败",
        resolutionOrder: "解析顺序：--api-key → auth.json → 环境变量 → models.json"
      },
    },
