...
impl ClaudeOwner<'_> {
    ...
    async fn fork_history_session(
        ...
        match self {
            Self::Facade(facade) => {
                facade
                    .fork_history_session(workspace_path, session_id, config)
                    .await
            }
            Self::Core(_) => {
                super::claude_history::fork_claude_session_with_config(
                    workspace_path,
                    session_id,
                    config,
                )
    ...
    async fn fork_history_session_from_message(
        &self,
        workspace_path: &Path,
        session_id: &str,
        message_id: &str,
        ...
        match self {
            Self::Facade(facade) => {
                facade
                    .fork_history_session_from_message(
                        workspace_path,
                        session_id,
                        message_id,
                        config,
            ...
            Self::Core(_) => {
                super::claude_history::fork_claude_session_from_message_with_config(
                    workspace_path,
                    session_id,
                    message_id,
                    config,
...
impl EngineManager {
    ...
    pub async fn fork_claude_history_session(
        &self,
        workspace_path: &Path,
        session_id: &str,
    ) -> Result<String, String> {
