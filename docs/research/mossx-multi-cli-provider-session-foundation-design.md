...
| Pi 图片输入 transport（2026-08-14 校准） | pi engine `image.input = supported`；headless 发送经 `@<abs path>` argv 走 pi 原生附件（旧 prompt 注入 transport 退役），history 解析 `<file name="...">` wrapper 与旧 `mossx:pi-image-attachments` 注入 marker 双格式，image blocks base64 不进投影 | `src-tauri/src/engine/{pi.rs,cli_image_input.rs,pi_history.rs}`、`openspec/specs/engine-capability-matrix/fixtures/matrix.json`、OpenSpec `upgrade-pi-image-input-transport` |
