...
| --- | --- | --- |
| engine registry（engine-neutral prewarm command 面） | 新增 `engine_prewarm(workspace_id, engine, session_id, provider_profile_id)`：按 engine 分发，pi 臂真实预热，其余引擎与 remote 模式 no-op `Ok(false)`；daemon 不注册（client-only，对齐 `prewarm_codex_disk_runtime` 先例） | `src-tauri/src/engine/commands.rs` `engine_prewarm`; `src-tauri/src/command_registry.rs`; OpenSpec `optimize-pi-first-packet-latency` |
| resident lifecycle（pi 预热） | `PiSession::prewarm_resident(session_id)` 仅接受合法 session id（恢复会话）——pending 会话 send scratch 每 turn 唯一（`pi_resident_map_key` 的 `scratch:<turn_id>`），预热 resident 无法被 send 命中，只会白起进程；幂等早退 + `rpc_disabled_blocks_spawn` 与 `ensure_resident` 同源；双轨契约：预热失败不影响首条发送的 `ensure_resident` 主路径；前端 `usePiResidentPrewarm` 挂 `useThreads`，pi 会话激活延迟 1.5s fire-and-forget、per-thread 双 ref 去重 | `src-tauri/src/engine/pi.rs` `prewarm_resident`; `src/features/threads/hooks/usePiResidentPrewarm.ts`; `src/services/tauri/appServer.ts` `enginePrewarm`（失败静默） |
