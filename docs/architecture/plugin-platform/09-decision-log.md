...
| D-051 | 2026-08-17 | 把 D-050 豁口扩成 **两根** 系统插头：`com.mossx.notes` + `com.mossx.engine.claude`。Claude 产品生命周期只用 `claude-worker` isolate；per-turn CLI 仍走 Process Entry。卸载后 `claude_commands_allowed()` 必须先于 `decide_claude_spawn_owner` 返回 `plugin-uninstalled`，禁止静默回 `cmd.spawn`。显式 `MOSSX_CLAUDE_PROCESS_ENTRY=0` 仍是 recovery。其余 10 根 later-plugin 仍只读 | 第二根闭环只能套已经走到 Disable-not-delete 的 Claude。地图 / 浏览器 / 画布仍是 0–1/9，禁止假装。证据：OpenSpec `plugin-rack-claude-install-loop`；`claude_lifecycle_activation_request()`；`install.rs` `claude_commands_allowed` |
| D-052 | 2026-08-17 | 把 D-051 豁口扩成 **三根** 系统插头：`com.mossx.notes` + `com.mossx.engine.claude` + `com.mossx.project-map`。地图产品生命周期用 fixture 全量 entries（worker/ui/memory-ui）。卸载后 `project_map_commands_allowed()` 必须先于 24 条产品 command 返回 `plugin-uninstalled`，禁止静默回 `*_core`。显式 `MOSSX_PROJECT_MAP_COMPAT_FACADE=0` 仍是 recovery。sqlite / checkpoint 保留。其余 9 根 later-plugin 仍只读；reject 目标改为 `com.mossx.browser` | 第三根闭环只能套已经走到产品默认隔离 + Disable-not-delete 的知识地图。浏览器 / 画布 / 其余 CLI 仍是 0/9，禁止假装。证据：OpenSpec `archive/2026-08-17-project-map-plugin-install-loop`；`project_map_activation_request()`；`install.rs` `project_map_commands_allowed` |
| D-053 | 2026-08-17 | 插排页改成可视化电源插排：live bank 恰好 3 座可插拔（Notes / Claude / Project Map），later bank 9 座封口且无安装按钮（含 disabled）。插座状态读 `desiredState`，不读 Host `state`。不改 snapshot API，不改 allowlist，不开 Marketplace | 卡片列表按 engine/feature 分组，无法验收「插排 100% / 插头 3 个可插拔」。本刀只是呈现层。证据：OpenSpec `plugin-rack-visual-strip`；`src/features/extensions/components/PluginRackSection.tsx`；原型 `docs/prototypes/plugin-rack-visual/index.html` |
| D-054 | 2026-08-18 | 三根 allowlist 插头走产品 LKG：每个 `pluginId` 在 `{app_home}/plugin-lock.json` 独立 pin（version + checkpointId + schemaVersion）。首次 `install_*` 在 slot Ready 后 `establish_own_lkg`；store 不健康则 `restore_pinned`；uninstall 不删 pin / sqlite / checkpoint。产品 `plugin-lockfile.json` 仍只管 desired-state。Claude health 只认 bookkeeping store + slot Ready，禁止假装会话 JSONL / schema-migrate。Host 仍 `enabled=false`。不接 Slim、不接 P2.6 / P2.7、不把 P2.5 整行勾完 | LKG 骨架只在测试里 `stage/complete`；产品 boot 曾用 ephemeral temp，pin 活不过重启。证据：OpenSpec `plugin-storage-lkg-product-path`；`plugin_runtime::{lkg,disk_storage,runtime,install,boot}`；`lib.rs` `boot_host_at(app_home)` |
| D-055 | 2026-08-17 | 允许市场页做本地 curated catalog UI，驱动 D-052 三根 allowlist 真装真卸。listing CTA 与插排共用 `PluginRackSnapshot.desiredState`。浏览器 `!isTauri()` 只改进程内存预览，拒绝 later-plugin，禁止 localStorage。远程 Registry / 签名 / 12 插头可写仍关。P6 不算完成 | D-049 禁的是 45 假插件 + localStorage 伪装市场，不是禁止 3 根真插头的商店壳。证据：OpenSpec `plugin-marketplace-local-catalog`；`src/features/extensions/components/{PluginRackSection,PluginMarketplaceCatalog}.tsx`；`src/services/tauri/pluginRack.ts` preview snapshot |
| D-056 | 2026-08-18 | Notes 完全体 last-mile：允许 `com.mossx.notes` 从本地独立仓 `install_plugin_from_path`。校验 `pluginId` 后把仓库 stage 到 `{storage_root}/plugin-runtime/plugins/com.mossx.notes/`，从 staged manifest 激活并 pin LKG。一键 Install 优先 staged，否则仍走 compile-time fixture。卸载 Disable-not-delete：pin + data + staged 都留。Slim 只转移 artifact/manifest 所有权，不删 `note_cards.rs` / Trusted React。Claude / Map 仍停在 7/9。产品代码禁止硬编码本机路径。远程 Registry / Host eval `dist/worker.js` 仍关 | 代码在 monorepo ≠ 假卸载，但 artifact 所有权没离开 Core 就定不了最后一公里。证据：OpenSpec `notes-slim-local-independent-repo`；`plugin_runtime/local_source.rs`；`install.rs` `install_plugin_from_path`；仓外 `mossx-plugin-notes` |
