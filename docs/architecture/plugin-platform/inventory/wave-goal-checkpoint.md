...
| 1AH max_concurrent 不得为 0 | 完成 | 本刀 |
...
相对「Core + 可撤销插件平台」全文约 **83%**（合同/插座组合面/disable+fuse+reset/storage 闸门/Host off/只读 Broker/失败激活必须 reset/并发上限/Ready swap/reset 撤 stream/disable 恢复/disable lifecycle/stream budget/未知 codec/重复 stream/其余 brokered 拒绝/provider 拒绝/跨插件 stream/deadline 下限/concurrent 下限）。  
相对「产品已拔插头」约 **0%**。  
