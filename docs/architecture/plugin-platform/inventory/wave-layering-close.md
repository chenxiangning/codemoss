...
| 市场入口 | 与拓展同级。10 根只读；**Notes + Claude 可写**（D-051） |
...
侧栏显示 12 个已声明 Host 插头。只有 `com.mossx.notes` 与 `com.mossx.engine.claude` 有安装/卸载按钮。没有 12 插头 Marketplace，没有 localStorage 假安装。
