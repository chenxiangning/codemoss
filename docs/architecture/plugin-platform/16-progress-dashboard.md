...
> 快照日期：**2026-08-17**
> 证据 change：`plugin-marketplace-local-catalog`（D-055 本地市场 UI）+ `plugin-product-surface-hide-on-uninstall`
> 验收：市场页 3 张可装/卸 listing + 9 张即将开放 + 插排状态条；卸后藏产品入口/面板；卸载 Claude 先确认再 interrupt。浏览器预览只改内存。远程 Marketplace 仍关。Host 仍 `enabled=false`。allowlist 仍 Notes + Claude + Project Map。
...
| 尺子 | 当前 | 缺口 | 分母是什么 |
|---|---:|---:|---|
| **允许线（Allowed line）** | **67%** | **33%** | `15` §3 走到 Disable + Notes/Claude/Map 真实 install/uninstall。Slim / 产品级 LKG / 远程 Marketplace 不计入。本地市场壳不抬这把尺子。 |
| **终态插件化（End-state）** | **14%** | **86%** | 独立仓库 + Slim + 产品级 LKG + Marketplace + 其余插头全部迁出。这是架构终态。 |
| **真实卸载（Real uninstall）** | **70%** | **30%** | 三根诚实闭环：lockfile + 产品闸门 + 藏入口/面板 + Claude interrupt。later-plugin 仍卸不掉。数据 Disable-not-delete，不是删 sqlite / 源码。 |
...
当前约束：**禁止第 8 步 Slim，禁止远程 Marketplace。** 本地 curated catalog（D-055）只演示已闭环的三根 allowlist 插头，不算 P6。插头协议允许线停在第 7 步 Disable-not-delete。第 8–9 步只出现在终态尺子里。
...
| P2 Lifecycle + Storage | **92%** | 8% | P2.1 atomic registry 已随 Notes 闭环落地。P2.4 破坏性 migration、P2.5 LKG、P2.6 retention、P2.7 crash 未收口。 |
...
| P0 下一刀 | Host 全局通电（真 boot，不再 `missing_executable()`） | 单列 | **高风险，另开 change。** 不是本刀。 |
| P1 | P2.5 LKG / P2.6 retention / P2.7 crash-during-migration | ~8% | 不把 P2 整行勾完。 |
...
当前刀尖
└── 用户目标（插排 100% + 3 真插头 + 市场可演示）已对齐。下一刀：Host 真 boot（高风险）或 Wave 2 LKG / P2.6 / P2.7。不 Slim，不开远程市场。
...
本地市场 UI + 卸后藏产品壳已落地。诚实下一刀：Host 真 boot（高风险另开）或 Wave 2 LKG / P2.6 / P2.7。不要开远程 Marketplace，不要 Slim，不要把 P6 或 P2.5 整行勾完。
...
| 目标块 | 现状 | 还差 | 何时算到 |
|---|---|---|---|
| 插排骨架（Wave 0–2 + 只读 12 卡） | **已够用** | Host 真 boot / 产品级 LKG / Marketplace **不进本目标** | 不阻塞可视化 |
...
| 尺子 | 现在 | 到目标后 | 仍不算进你的目标 |
|---|---:|---:|---|
| 允许线 | **67%** | **~67%** | Host 真 boot、产品级 LKG |
| 真实卸载 | **70%** | **~70%** | 另外 9 根仍卸不掉；数据 Disable-not-delete |
| 终态 | 14% | **仍 ~14%** | Slim / 独立仓 / 远程 Marketplace |
