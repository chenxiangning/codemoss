...
> 快照日期：**2026-08-17**
> 证据 change：`plugin-rack-visual-strip`（D-053 可视化插排 3/9）
> 验收：插排页 live bank 3 座可插拔 + later bank 9 座封口无按钮。Marketplace 仍关。allowlist 仍 Notes + Claude + Project Map。
...
| 尺子 | 当前 | 缺口 | 分母是什么 |
|---|---:|---:|---|
| **允许线（Allowed line）** | **67%** | **33%** | `15` §3 走到 Disable + Notes/Claude/Map 真实 install/uninstall。Slim / LKG / Marketplace 不计入。可视化插排已落地，不抬这把尺子。 |
| **终态插件化（End-state）** | **14%** | **86%** | 独立仓库 + Slim + LKG + Marketplace + 其余插头全部迁出。这是架构终态。 |
| **真实卸载（Real uninstall）** | **60%** | **40%** | Notes + Claude + Project Map 三根诚实闭环。later-plugin 仍卸不掉。Claude 生命周期是 worker isolate，per-turn CLI 仍 Process Entry。 |

读法：

- 问「这波还能干什么」→ 看允许线 67%。
- 问「离可安装/可卸载的插件生态还有多远」→ 看终态 14%。
- 问「现在能不能从产品里拔掉 Claude / Notes / 知识地图」→ 看真实卸载 60%（三根都能拔；插排页 3 座可插拔 / 9 座封口）。
...
| Rack UI | **~100%** | 0% | 可视化插排：3 座可插拔 + 9 座封口。远程 Marketplace 仍关。口径是 3/9，不是 12 根全可写。 |
...
| 优先级 | 缺口 | 约占允许线 | 状态 |
|---|---|---:|---|
| **刚落地** | 可视化插排 3 真 / 9 封口 | 单列 | `plugin-rack-visual-strip` / D-053。Rack UI ~100%（3/9 口径）。 |
| P0 下一刀 | Wave 2 收口：破坏性 migration、LKG 骨架 | ~6% | 用户目标块已完成。不在本目标内。 |
| P1 | Host 全局通电（真 boot，不再 `missing_executable()`） | 单列 | **高风险，另开 change。** 不是本刀。 |
| 冻结 | Slim Claude / Notes / 任何 Core 实现 | 终态 | **禁止。** 缺口 1y。 |
| 冻结 | Marketplace / 12 插头可写 / 独立仓库发布 | 终态 | **禁止。** D-049 仍有效；D-052 只豁 Notes + Claude + Project Map。 |
...
└── 可视化插排：live 3 可插拔 / later 9 封口（D-053）

当前刀尖
└── 用户目标块已完成：插排 100%（3/9 口径），三根可插拔。下一刀不在本目标：Wave 2 LKG / Host 真 boot。
...
可视化插排已落地（3 真 / 9 封口）。用户目标块完成。诚实下一刀不在本目标内：Wave 2 LKG / Host 真 boot。不要开 Marketplace，不要 Slim。
...
| 尺子 | 现在 | 到目标后 | 仍不算进你的目标 |
|---|---:|---:|---|
| 允许线 | **67%** | **~67%** | Wave 2 LKG、Host 真 boot |
| 真实卸载 | **60%** | **~60%** | 另外 9 根仍卸不掉 |
| 终态 | 14% | **仍 ~14%** | Slim / 独立仓 / Marketplace |
| Rack 可视化 | **~100%（3/9 口径）** | **~100%（3/9 口径）** | 远程市场、12 根全可写 |
