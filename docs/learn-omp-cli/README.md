...
| Crate (库名) | 行数 | 干什么的 |
| -------------- | -----: | --------- |
| `pi-shell` | ~38k | brush bash fork + 58 coreutils (核心 Unix 工具集) 内置 |
| `pi-natives` | ~25k | N-API (Node ↔ Rust 桥) 表面 |
| `pi-walker` | ~5.2k | 并行 ignore-aware (忽略感知) 遍历器 |
| `pi-iso` | ~3.3k | workspace 隔离(apfs/btrfs/zfs/reflink/overlayfs/projfs) |
| `pi-ast` | ~2.9k | tree-sitter (语法解析器) + ast-grep |
...
| `advisor` | 06 旁听模型 |
| `vision` / `designer` / `task` / `tiny` | 各司其职 |
