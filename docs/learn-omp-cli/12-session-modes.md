...
director 自己是**只读**,只能 read/grep/glob/todo,**不能 write/edit/bash**——保证你的代码不被 director 直接碰。

### 1.2 进入方式

```text
/vibe               # 进入 vibe mode
/vibe fast          # 进 vibe + 配 fast role(便宜模型 worker)
/vibe good          # 进 vibe + 配 good role(强模型 worker)
/vibe exit          # 退出
```
...
# 内部效果:
1. 清掉 provider 那边的 prompt cache 状态
2. 重置流式输出 buffer
3. 保留本地 transcript (本地对话转录文件, 不动你的历史)
5. 等价于"心跳重启",但你不用退出 session
```
...
```text
/model                           # 列出当前 role 的可选模型
/model gpt-5.5                   # 切 default 到 gpt-5.5
/model opus                      # 切 default 到 claude-opus-4.7 (缩写匹配)
/model --smol haiku              # 切 smol role 到 haiku
/model --plan opus               # 切 plan role 到 opus
```
...
> `Ctrl+P`:在当前 role 的几个候选里转(类似 alt-tab)
> `/model`:全角色重选(类似 Windows 切桌面)
