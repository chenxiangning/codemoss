...
modelRoles:
  default:    claude-sonnet        # 主流
  advisor:    claude-haiku         # 每回合旁听,必须够便宜
  reasoning:  claude-opus          # 复杂多步推理
```

启动后,advisor 会在每个 turn 末尾插入一小段"潜在风险/改进"——不打断主任务流。

**期望**:

- 主任务结束后,自动看到 advisor 给的两三句提示;
- 关掉 advisor → `modelRoles.advisor: null`;
- advisor 看的是完整上下文(不是只读 stub),所以"它说的话"有依据。

**踩坑提醒**:

- advisor 太贵的模型会让你"每回合都烧钱",别用 opus 当 advisor;
- advisor ≠ 安全审计 —— 它只是旁听,真要严审用 `/review`。
...
verdict:
  ...
  confidence: 0.0 ~ 1.0   # 评审模型对自己的判断有多大把握
  findings:
...
- `decision` 是结构化的,不是空话;
- `confidence` 低于阈值(默认 0.7)时,TUI 会再问你"要不要继续";
- `/review --strict` 是"任何 major 都阻断 PR",`/review --soft` 是"只报告、不阻断"。
...
| 维度 | pi | omp |
| ------ | ----- | ----- |
| 并行子代理 | 没原生,靠外部脚本 | `task { workers: N, schema }` |
...
1. 用 `task { workers: 3, jobs: [...] }` 派并行子代理,每人 schema-validated (格式校验过的) JSON。
2. 给 `~/.omp/agent/config.yml` 配 `advisor` 角色,挑便宜模型。
3. 用 `/review` 拿 verdict + confidence,低于阈值就停下。
