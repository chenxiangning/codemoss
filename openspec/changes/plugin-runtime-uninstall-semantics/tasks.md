...
- [x] 1.7 `cargo test --lib plugin_runtime::host`（27 passed）+ `cargo test --lib plugin_runtime::spawn`（23 passed）+ `cargo test --lib plugin_runtime`（287 passed）
- [x] 1.8 确认 `boot_driver()` 与生产引擎未被改动
