...
- [x] 1.9 最完整端到端闭环：真实 peer + `PluginRuntime` 组合 uninstall 杀进程组 + 撤销三类 composed handles + 不可恢复（`runtime_uninstall_stops_real_peer_and_revokes_composed_handles`）
- [x] 1.10 `cargo test --lib plugin_runtime::host`（27 passed）+ `cargo test --lib plugin_runtime::spawn`（24 passed）+ `cargo test --lib plugin_runtime`（290 passed）
- [x] 1.11 确认 `boot_driver()` 与生产引擎未被改动
