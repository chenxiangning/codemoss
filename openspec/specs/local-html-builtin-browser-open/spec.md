# local-html-builtin-browser-open Specification

## Purpose

Defines how the client opens local HTML files in the built-in Browser Agent (not the system browser), including entry surfaces, path encoding, eligibility, and user-facing error semantics.
...
- **AND** the client MUST surface a non-blocking global error toast
