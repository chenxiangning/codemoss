# project-map-plugin-install-loop-v1 Specification

## Purpose

产品插排对第三根 allowlisted 系统插头 `com.mossx.project-map` 提供真实安装 / 卸载闭环：disk lockfile 跨重启、fixture 全量 entries 的 Host Ready、原子 `project-map.main` + `project-map.memory` contribution、卸载后 24 条 command 闸门先于 facade。本 capability 不 Slim、不开 Marketplace、不给 later-plugin 装假按钮。
