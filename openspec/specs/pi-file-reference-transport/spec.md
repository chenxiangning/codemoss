# pi-file-reference-transport Specification

## Purpose

Defines how the Pi engine runtime transports `@<path>` file reference tokens embedded in the composer prompt text: extraction into native `@file` argv arguments, degradation for unresolvable tokens, and the argv guard that prevents the Pi CLI from misparsing the whole prompt as a file argument.
