# conversation-tool-output-budget Specification

## Purpose

会话态工具输出（`commandExecution` / `fileChange`）在写入 `ConversationItem.output` 之前 MUST 经过 head+tail 字节预算，防止无界 exec 输出把 JS heap / WebView 打穿。事件送达契约归 `app-server-event-stream-pacing` 管；本条只管组装后正文的大小上限。
