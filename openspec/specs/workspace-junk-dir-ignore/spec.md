# workspace-junk-dir-ignore Specification

## Purpose

垃圾目录（依赖 + 构建产物，含 `temp` / `tmp` / `.tmp`）在 workspace listing、file tree、project-map builtin ignore 与 Codex thread start 的 `.codexignore` 托管段中 MUST 保持同一事实源名单，降低引擎扫描依赖 / 构建目录的概率。
