...
产品插排对 allowlisted 系统插头提供真实安装 / 卸载闭环：disk lockfile 跨重启、Host Ready via `activate_allowlisted`、原子 contribution、卸载终态不可静默回 Core。本 capability 覆盖 Notes（D-050）与 Claude（D-051）；其它 later-plugin 不得假装可装。
