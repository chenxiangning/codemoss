...
| Implementation rules | `dev-guidelines/**` | frontend/backend/guides 的实现规范与执行约束 | 不要承担 active change 审计历史 |
...
1. 先读 `AGENTS.md`
2. 根据任务类型按需读：
   - 实现规则：`dev-guidelines/frontend/index.md` / `dev-guidelines/backend/index.md`
   - 思考指南：`dev-guidelines/guides/index.md`
   - 行为与变更：`openspec/README.md`、`openspec/project.md`、对应 `openspec/changes/<change-id>/**`
3. 只有调试 host 行为时，再深入 `.claude/**` 或 `.codex/**`

### 修改规则时

- 改 frontend/backend/cross-layer 约束：去 `dev-guidelines/**`
...
| frontend hook / state / i18n 规则 | `dev-guidelines/frontend/**` | `AGENTS.md` |
| backend command / storage / logging 规则 | `dev-guidelines/backend/**` | `AGENTS.md` |
...
- 在 `AGENTS.md` 重新复制 `dev-guidelines/frontend/**` 或 `dev-guidelines/backend/**` 的细节
